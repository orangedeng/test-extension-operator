...
import (
	...
	resources "github.com/orangedeng/test-extension-operator/pkg/apis/test.pandaria.io/v1"
...
func WriteCRD() error {
	for _, crdDef := range List() {
		...
		filename := fmt.Sprintf("./charts/test-extension-crd/templates/%s.yaml", strings.ToLower(crd.Spec.Names.Kind))
...
func List() []crd.CRD {
	return []crd.CRD{
		...
		// 	return c
		// }),
		newCRD(resources.Foo{}, func(c crd.CRD) crd.CRD {
			return c
		}),
	}
}
